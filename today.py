...
if __name__ == '__main__':
    ...
    if 'BIRTH_YEAR' in os.environ:
        birth_date = datetime.datetime(
            int(os.environ['BIRTH_YEAR']),
            int(os.environ.get('BIRTH_MONTH', 1)),
            int(os.environ.get('BIRTH_DAY', 1))
        )
    else:
        # Default to GitHub account creation date if real birthday is not specified
        birth_date = datetime.datetime.strptime(acc_date, '%Y-%m-%dT%H:%M:%SZ')
